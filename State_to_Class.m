function [ E1,I,Err ] = State_to_Class( a,b,ProbStop, N,kB,kV,kL,TB,TV,TL,ticker, maxAge, points)
...
%point ticker. It gives the individual terms which when summed give E,
...
if Nc==0
    E1 = 0; I = 0; Err = 0; % No kids, everything is zero
else
    Nc_max = find(ProbStop==1,1)-2;
    if Nc>(ticker<=kB+kV+kL+kB)*(Nc_max-(ticker>kB+kV+kL))
        E1 = 0; I = 0; Err = 0; % If we have more children than allowed
        disp(['Demographic state impossible under parameters, #children=' num2str(Nc) ]);
    elseif ticker>kB+kV+kL+kB
        E1 = 0; I = 0; Err = 0; % If all children should have left
        disp('Children gone');
    else
        ...
        for i=1:Nc-1 % Cycle over all but youngest child
            ...
            for p=1:points
                ...
                if (Y_sample(1)<b-sum(Y_sample(2:end))) && (Y_sample(1)>a-sum(Y_sample(2:end)))
                    P = IntervalProb(Y_sample,ProbStop,kB,kV,kL,TB,TV,TL,ticker);
                    samples(p) = P;
                end
            end
            I(i) = V*mean(samples);
...
function [P] = IntervalProb(Y,P_Stop,kB,kV,kL,TB,TV,TL,ticker)
...
elseif ticker<kB+kV+1 % Waiting for youngest to get vaccinated
    p_last = gampdf(Y(Nc),ticker-kB,TV/kV); % Erlang again  % Exponential but sure call it erlang
elseif ticker<kB+kV+kL+1 % Waiting for first one to leave
    p_last = gampdf(Y(Nc),ticker-kB-kV,(TL- (Nc-1)*TB)/kL); % Erlang again      % maybe the scale parameter should be (TL- (Nc-1)*TB)/kL
elseif ticker<kB+kV+kL+kB+1 % Waiting for others to leave
    p_last = 0;
    if N_max>1
        Size_Is = zeros(1,N_max-Nc);
        for j=1:N_max-Nc % Nc+j gives total number of children to be born   
            % j is M from the formula
            Size_Is(j)=P_Stop(Nc+j+2); % Probability that total number of kids was N_max-(i-j)
            for l=0:Nc+j-1
                Size_Is(j)=Size_Is(j)*(1-P_Stop(l+2));  % This is taking care of the summation in the formula
            end
            lambda_L1 = (kV/TV)*ones(1,kV);
            lambda_L2 = (kL/TL)*ones(1,kL);
            lambda_L3 = (kB/TB)*ones(1,ticker-kB-kL+kB*(j-1));
            lambda = [lambda_L1 lambda_L2 lambda_L3];
